"""Kaggle T4 entrypoint for bounded two-sample calibration."""
...
logging.basicConfig(
    level=logging.INFO,
    format="[GPU Multiframe Calibration] %(asctime)s - %(levelname)s: %(message)s",
)
logger = logging.getLogger("GPUMultiframeCalibration")

if not os.path.exists("/kaggle/input"):
    raise RuntimeError("GPU multiframe calibration must run inside Kaggle")
...
import src.gpu_multiframe_calibration  # noqa: E402
...
from src.gpu_multiframe_calibration import run_gpu_multiframe_calibration  # noqa: E402
...
modules_to_verify = [
    ...
    src.gpu_multiframe_calibration,
    src.model,
    src.train,
]
...
output_dir = Path("/kaggle/working/gpu_multiframe_calibration")
...
logger.info("Calibration execution verdict: %s", report["execution_verdict"])
logger.info("Scientific result: %s", report.get("scientific_result"))
logger.info("Report: %s", output_dir / "gpu_multiframe_calibration_report.json")
if report["execution_verdict"] != "PASS":
    raise RuntimeError(f"GPU multiframe calibration failed: {report['failure_reasons']}")
