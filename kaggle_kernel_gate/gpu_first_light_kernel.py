"""Kaggle T4 entrypoint for the bounded ST-ACT GPU first-light gate."""
...
logging.basicConfig(level=logging.INFO, format="[GPU First Light] %(asctime)s - %(levelname)s: %(message)s")
logger = logging.getLogger("GPUFirstLight")

if not os.path.exists("/kaggle/input"):
    raise RuntimeError("GPU first-light kernel must run inside Kaggle")
...
logger.info("Deployed code SHA: %s", deployed_sha)

# Copy of the known-good training-kernel install contract: --no-deps protects
# Kaggle's numpy/scipy, while explicit companions prevent silent missing runtimes.
subprocess.run(
    [
        sys.executable,
        "-m",
...
import src.gpu_first_light_gate  # noqa: E402
import src.model  # noqa: E402
import src.split_utils  # noqa: E402
import src.targets  # noqa: E402
import src.train  # noqa: E402
from src.deployment_provenance import verify_import_origins  # noqa: E402
from src.gpu_first_light_gate import run_gpu_first_light_gate  # noqa: E402
...
import_origins = {module.__name__: str(Path(module.__file__).resolve()) for module in modules_to_verify}

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
cuda_arch_compatible = False
if device.type == "cuda":
    major, minor = torch.cuda.get_device_capability(device)
    cuda_arch_compatible = f"sm_{major}{minor}" in torch.cuda.get_arch_list()

entrypoint_sha256 = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
competition_train_dir = Path("/kaggle/input/competitions/biohub-cell-tracking-during-development/train")
split_file = resolve_split_file_path(kaggle_src_dataset_dir=source_root)
output_dir = Path("/kaggle/working/gpu_first_light")
...
report = run_gpu_first_light_gate(
    ...
    gate_entrypoint_sha256=entrypoint_sha256,
    cuda_arch_compatible=cuda_arch_compatible,
)
logger.info("GPU first-light verdict: %s", report["verdict"])
logger.info("Report: %s", output_dir / "gpu_first_light_report.json")
if report["verdict"] != "PASS":
    raise RuntimeError(f"GPU first-light gate failed: {report['failure_reasons']}")
