"""Kaggle T4 entrypoint for the bounded ST-ACT GPU learning probe."""
...
logging.basicConfig(level=logging.INFO, format="[GPU Learning Probe] %(asctime)s - %(levelname)s: %(message)s")
logger = logging.getLogger("GPULearningProbe")

if not os.path.exists("/kaggle/input"):
    raise RuntimeError("GPU learning probe kernel must run inside Kaggle")
...
import src.gpu_learning_probe  # noqa: E402
...
from src.gpu_learning_probe import run_gpu_learning_probe  # noqa: E402
...
output_dir = Path("/kaggle/working/gpu_learning_probe")
...
report = run_gpu_learning_probe(
    ...
    probe_entrypoint_sha256=entrypoint_sha256,
    cuda_arch_compatible=cuda_arch_compatible,
)
logger.info("GPU learning probe verdict: %s", report["verdict"])
logger.info("Learning signal observed: %s", report["learning_signal_observed"])
logger.info("Report: %s", output_dir / "gpu_learning_probe_report.json")
if report["verdict"] != "PASS":
    raise RuntimeError(f"GPU learning probe failed: {report['failure_reasons']}")
